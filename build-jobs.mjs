...
section = section.replace('<tbody id="jobRows"></tbody>', '<tbody id="jobRows">' + jobs.map(job => `<tr data-department="${escape(job[0])}" data-type="${escape(job[1])}"><td><span class="tag">${escape(job[1])}</span></td><td>${escape(job[2])}</td><td class="dept">${escape(job[0])}</td><td>${escape(job[3])}</td><td><img class="job-qr" src="${job[4]}" alt="${escape(job[2])}二维码" loading="eager"></td></tr>`).join('') + '</tbody>');
for (const [id, index] of [['department',0], ['type',1]]) {
  section = section.replace(new RegExp(`(<select id="${id}"[^>]*>[\\s\\S]*?)(</select>)`), (_, start, end) => start + [...new Set(jobs.map(job => job[index]))].map(value => `<option value="${escape(value)}">${escape(value)}</option>`).join('') + end);
}
...
script = `(() => {
  const rows = Array.from(document.querySelectorAll('#jobRows tr'));
  ...
  function filter() {
    ...
    let visible = 0;
    rows.forEach(row => {
      row.hidden = !((!query || row.textContent.toLowerCase().includes(query)) && (!department.value || row.dataset.department === department.value) && (!type.value || row.dataset.type === type.value));
      if (!row.hidden) visible++;
    });
    document.querySelector('#count').textContent = visible + '个岗位';
  }
  [search, department, type].forEach(control => control.addEventListener('input', filter));
  filter();
})();`;
const css = `
